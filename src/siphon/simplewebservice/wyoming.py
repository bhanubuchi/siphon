...
class WyomingUpperAir(HTTPEndPoint):
    ...
    def _get_data(self, time, site_id):
        ...
        col_names = ['pressure', 'height', 'temperature', 'dewpoint', 'direction', 'speed', 'rh']
        df = pd.read_fwf(tabular_data, widths=[7] * 8, skiprows=5,
                         usecols=[0, 1, 2, 3, 4, 5, 6, 7], names=col_names)
        ...
        df = df.dropna(subset=('temperature', 'dewpoint', 'direction', 'speed',
                               'u_wind', 'v_wind', 'rh'), how='all').reset_index(drop=True)
